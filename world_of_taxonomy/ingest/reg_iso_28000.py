"""Ingest ISO 28000:2022 Security and Resilience - Security Management Systems."""
...
_SOURCE_URL = "https://www.iso.org/standard/79612.html"
...
NODES: list[tuple[str, str, int, str | None]] = [
    ...
    ("4_3", "4.3 Determining the scope of the security management system", 2, "cl_4"),
    ("5_1", "5.1 Leadership and commitment", 2, "cl_5"),
    ("5_2", "5.2 Security management policy", 2, "cl_5"),
    ("5_3", "5.3 Roles, responsibilities and authorities", 2, "cl_5"),
    ("6_1", "6.1 Actions to address risks and opportunities", 2, "cl_6"),
    ("6_2", "6.2 Security objectives and plans to achieve them", 2, "cl_6"),
    ("8_1", "8.1 Operational planning and control", 2, "cl_8"),
    ("8_2", "8.2 Security risk assessment", 2, "cl_8"),
    ("8_3", "8.3 Security risk treatment", 2, "cl_8"),
    ("8_4", "8.4 Security plans", 2, "cl_8"),
...
async def ingest_reg_iso_28000(conn) -> int:
    """Insert or update ISO 28000:2022 system and its nodes. Returns node count."""
    await conn.execute(
    ...
    await conn.execute(
        "DELETE FROM classification_node WHERE system_id = $1", "reg_iso_28000"
    )
    for code, title, level, parent_code in NODES:
        await conn.execute(
            """INSERT INTO classification_node (system_id, code, title, level, parent_code)
               VALUES ($1, $2, $3, $4, $5)""",
            "reg_iso_28000", code, title, level, parent_code,
        )
    count = len(NODES)
    await conn.execute(
