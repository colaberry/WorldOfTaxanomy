"""Tests for HITRUST CSF regulatory taxonomy ingester.
...
from world_of_taxonomy.ingest.reg_hitrust import (
    REG_HITRUST_NODES,
    ingest_reg_hitrust,
)


class TestRegHitrustNodes:
    def test_total_node_count(self):
        assert len(REG_HITRUST_NODES) == 27
    ...
    def test_no_duplicate_codes(self):
        codes = [n[0] for n in REG_HITRUST_NODES]
    ...
    def test_has_level_1_nodes(self):
        l1 = [n for n in REG_HITRUST_NODES if n[2] == 1]
        assert len(l1) >= 1


def test_reg_hitrust_module_importable():
    assert callable(ingest_reg_hitrust)
    assert isinstance(REG_HITRUST_NODES, list)


def test_ingest_reg_hitrust(db_pool):
    """Integration test: ingest HITRUST CSF taxonomy."""
    async def _run():
...
def test_ingest_reg_hitrust_idempotent(db_pool):
    """Running ingest twice returns same count."""
    async def _run():
        async with db_pool.acquire() as conn:
            count1 = await ingest_reg_hitrust(conn)
            count2 = await ingest_reg_hitrust(conn)
