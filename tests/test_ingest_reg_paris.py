"""Tests for reg_paris ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.reg_paris import NODES, ingest_reg_paris


class TestRegParisNodes:
...
def test_reg_paris_module_importable():
    assert callable(ingest_reg_paris)


def test_ingest_reg_paris(db_pool):
    async def _run():
...
def test_ingest_reg_paris_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_reg_paris(conn)
            count = await ingest_reg_paris(conn)
            assert count == 20
    asyncio.get_event_loop().run_until_complete(_run())
