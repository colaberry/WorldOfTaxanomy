...
class TestIngestLcc:
    ...
    def test_ingest_creates_system(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count = await ingest_lcc(conn)
                assert count >= 80, f"Expected >= 80 nodes, got {count}"

                sys = await _get_system(conn, "lcc")
                assert sys is not None
                assert sys["authority"] == "Library of Congress"
    ...
    def test_ingest_creates_nodes(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count = await ingest_lcc(conn)
                db_count = await _count_nodes(conn, "lcc")
    ...
    def test_idempotent(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count1 = await ingest_lcc(conn)
                count2 = await ingest_lcc(conn)
                db_count = await _count_nodes(conn, "lcc")
