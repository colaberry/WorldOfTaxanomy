"""Tests for reg_iso_22000 ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.reg_iso_22000 import NODES, ingest_reg_iso_22000


class TestRegIso22000Nodes:
...
def test_reg_iso_22000_module_importable():
    assert callable(ingest_reg_iso_22000)


def test_ingest_reg_iso_22000(db_pool):
    async def _run():
...
def test_ingest_reg_iso_22000_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_reg_iso_22000(conn)
            count = await ingest_reg_iso_22000(conn)
            assert count == 31
    asyncio.get_event_loop().run_until_complete(_run())
