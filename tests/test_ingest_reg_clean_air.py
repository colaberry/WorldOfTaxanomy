"""Tests for Clean Air Act regulatory taxonomy ingester.
...
from world_of_taxonomy.ingest.reg_clean_air import (
    REG_CLEAN_AIR_NODES,
    ingest_reg_clean_air,
)


class TestRegCleanAirNodes:
    def test_total_node_count(self):
        assert len(REG_CLEAN_AIR_NODES) == 28
    ...
    def test_no_duplicate_codes(self):
        codes = [n[0] for n in REG_CLEAN_AIR_NODES]
    ...
    def test_has_level_1_nodes(self):
        l1 = [n for n in REG_CLEAN_AIR_NODES if n[2] == 1]
        assert len(l1) >= 1


def test_reg_clean_air_module_importable():
    assert callable(ingest_reg_clean_air)
    assert isinstance(REG_CLEAN_AIR_NODES, list)


def test_ingest_reg_clean_air(db_pool):
    """Integration test: ingest Clean Air Act taxonomy."""
    async def _run():
...
def test_ingest_reg_clean_air_idempotent(db_pool):
    """Running ingest twice returns same count."""
    async def _run():
        async with db_pool.acquire() as conn:
            count1 = await ingest_reg_clean_air(conn)
            count2 = await ingest_reg_clean_air(conn)
