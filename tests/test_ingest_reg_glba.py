"""Tests for GLBA regulatory taxonomy ingester.
...
from world_of_taxonomy.ingest.reg_glba import (
    REG_GLBA_NODES,
    ingest_reg_glba,
)


class TestRegGlbaNodes:
    def test_total_node_count(self):
        assert len(REG_GLBA_NODES) == 28
    ...
    def test_no_duplicate_codes(self):
        codes = [n[0] for n in REG_GLBA_NODES]
    ...
    def test_has_level_1_nodes(self):
        l1 = [n for n in REG_GLBA_NODES if n[2] == 1]
        assert len(l1) >= 1


def test_reg_glba_module_importable():
    assert callable(ingest_reg_glba)
    assert isinstance(REG_GLBA_NODES, list)


def test_ingest_reg_glba(db_pool):
    """Integration test: ingest GLBA taxonomy."""
    async def _run():
...
def test_ingest_reg_glba_idempotent(db_pool):
    """Running ingest twice returns same count."""
    async def _run():
        async with db_pool.acquire() as conn:
            count1 = await ingest_reg_glba(conn)
            count2 = await ingest_reg_glba(conn)
