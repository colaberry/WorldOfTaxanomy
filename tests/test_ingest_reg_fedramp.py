"""Tests for FedRAMP regulatory taxonomy ingester.
...
from world_of_taxonomy.ingest.reg_fedramp import (
    REG_FEDRAMP_NODES,
    ingest_reg_fedramp,
)


class TestRegFedrampNodes:
    def test_total_node_count(self):
        assert len(REG_FEDRAMP_NODES) == 40
    ...
    def test_no_duplicate_codes(self):
        codes = [n[0] for n in REG_FEDRAMP_NODES]
    ...
    def test_has_level_1_nodes(self):
        l1 = [n for n in REG_FEDRAMP_NODES if n[2] == 1]
        assert len(l1) >= 1


def test_reg_fedramp_module_importable():
    assert callable(ingest_reg_fedramp)
    assert isinstance(REG_FEDRAMP_NODES, list)


def test_ingest_reg_fedramp(db_pool):
    """Integration test: ingest FedRAMP taxonomy."""
    async def _run():
...
def test_ingest_reg_fedramp_idempotent(db_pool):
    """Running ingest twice returns same count."""
    async def _run():
        async with db_pool.acquire() as conn:
            count1 = await ingest_reg_fedramp(conn)
            count2 = await ingest_reg_fedramp(conn)
