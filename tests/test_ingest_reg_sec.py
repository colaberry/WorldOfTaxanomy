"""Tests for SEC Regulations regulatory taxonomy ingester.
...
from world_of_taxonomy.ingest.reg_sec import (
    REG_SEC_NODES,
    ingest_reg_sec,
)


class TestRegSecNodes:
    def test_total_node_count(self):
        assert len(REG_SEC_NODES) == 29
    ...
    def test_no_duplicate_codes(self):
        codes = [n[0] for n in REG_SEC_NODES]
    ...
    def test_has_level_1_nodes(self):
        l1 = [n for n in REG_SEC_NODES if n[2] == 1]
        assert len(l1) >= 1


def test_reg_sec_module_importable():
    assert callable(ingest_reg_sec)
    assert isinstance(REG_SEC_NODES, list)


def test_ingest_reg_sec(db_pool):
    """Integration test: ingest SEC Regulations taxonomy."""
    async def _run():
...
def test_ingest_reg_sec_idempotent(db_pool):
    """Running ingest twice returns same count."""
    async def _run():
        async with db_pool.acquire() as conn:
            count1 = await ingest_reg_sec(conn)
            count2 = await ingest_reg_sec(conn)
