"""Tests for reg_iso_19011 ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.reg_iso_19011 import NODES, ingest_reg_iso_19011


class TestRegIso19011Nodes:
...
def test_reg_iso_19011_module_importable():
    assert callable(ingest_reg_iso_19011)


def test_ingest_reg_iso_19011(db_pool):
    async def _run():
...
def test_ingest_reg_iso_19011_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_reg_iso_19011(conn)
            count = await ingest_reg_iso_19011(conn)
            assert count == 30
    asyncio.get_event_loop().run_until_complete(_run())
