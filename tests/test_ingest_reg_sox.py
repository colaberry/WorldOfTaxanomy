"""Tests for SOX regulatory taxonomy ingester.
...
from world_of_taxonomy.ingest.reg_sox import (
    REG_SOX_NODES,
    ingest_reg_sox,
)


class TestRegSoxNodes:
    def test_total_node_count(self):
        assert len(REG_SOX_NODES) == 58
    ...
    def test_no_duplicate_codes(self):
        codes = [n[0] for n in REG_SOX_NODES]
    ...
    def test_has_level_1_nodes(self):
        l1 = [n for n in REG_SOX_NODES if n[2] == 1]
        assert len(l1) >= 1


def test_reg_sox_module_importable():
    assert callable(ingest_reg_sox)
    assert isinstance(REG_SOX_NODES, list)


def test_ingest_reg_sox(db_pool):
    """Integration test: ingest SOX taxonomy."""
    async def _run():
...
def test_ingest_reg_sox_idempotent(db_pool):
    """Running ingest twice returns same count."""
    async def _run():
        async with db_pool.acquire() as conn:
            count1 = await ingest_reg_sox(conn)
            count2 = await ingest_reg_sox(conn)
            assert count1 == count2 == 58
