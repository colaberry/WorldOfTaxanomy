"""Tests for SOC 2 Trust Criteria regulatory taxonomy ingester.
...
from world_of_taxonomy.ingest.reg_soc2 import (
    REG_SOC2_NODES,
    ingest_reg_soc2,
)


class TestRegSoc2Nodes:
    def test_total_node_count(self):
        assert len(REG_SOC2_NODES) == 37
    ...
    def test_no_duplicate_codes(self):
        codes = [n[0] for n in REG_SOC2_NODES]
    ...
    def test_has_level_1_nodes(self):
        l1 = [n for n in REG_SOC2_NODES if n[2] == 1]
        assert len(l1) >= 1


def test_reg_soc2_module_importable():
    assert callable(ingest_reg_soc2)
    assert isinstance(REG_SOC2_NODES, list)


def test_ingest_reg_soc2(db_pool):
    """Integration test: ingest SOC 2 Trust Criteria taxonomy."""
    async def _run():
...
def test_ingest_reg_soc2_idempotent(db_pool):
    """Running ingest twice returns same count."""
    async def _run():
        async with db_pool.acquire() as conn:
            count1 = await ingest_reg_soc2(conn)
            count2 = await ingest_reg_soc2(conn)
