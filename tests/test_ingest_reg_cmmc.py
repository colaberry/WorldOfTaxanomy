"""Tests for CMMC 2.0 regulatory taxonomy ingester.
...
from world_of_taxonomy.ingest.reg_cmmc import (
    REG_CMMC_NODES,
    ingest_reg_cmmc,
)


class TestRegCmmcNodes:
    def test_total_node_count(self):
        assert len(REG_CMMC_NODES) == 25
    ...
    def test_no_duplicate_codes(self):
        codes = [n[0] for n in REG_CMMC_NODES]
    ...
    def test_has_level_1_nodes(self):
        l1 = [n for n in REG_CMMC_NODES if n[2] == 1]
        assert len(l1) >= 1


def test_reg_cmmc_module_importable():
    assert callable(ingest_reg_cmmc)
    assert isinstance(REG_CMMC_NODES, list)


def test_ingest_reg_cmmc(db_pool):
    """Integration test: ingest CMMC 2.0 taxonomy."""
    async def _run():
...
def test_ingest_reg_cmmc_idempotent(db_pool):
    """Running ingest twice returns same count."""
    async def _run():
        async with db_pool.acquire() as conn:
            count1 = await ingest_reg_cmmc(conn)
            count2 = await ingest_reg_cmmc(conn)
