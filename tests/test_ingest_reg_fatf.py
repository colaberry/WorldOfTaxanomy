"""Tests for reg_fatf ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.reg_fatf import NODES, ingest_reg_fatf


class TestRegFatfNodes:
...
def test_reg_fatf_module_importable():
    assert callable(ingest_reg_fatf)


def test_ingest_reg_fatf(db_pool):
    async def _run():
...
def test_ingest_reg_fatf_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_reg_fatf(conn)
            count = await ingest_reg_fatf(conn)
            assert count == 29
    asyncio.get_event_loop().run_until_complete(_run())
