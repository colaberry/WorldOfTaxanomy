"""Tests for reg_iso_22313 ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.reg_iso_22313 import NODES, ingest_reg_iso_22313


class TestRegIso22313Nodes:
...
def test_reg_iso_22313_module_importable():
    assert callable(ingest_reg_iso_22313)


def test_ingest_reg_iso_22313(db_pool):
    async def _run():
...
def test_ingest_reg_iso_22313_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_reg_iso_22313(conn)
            count = await ingest_reg_iso_22313(conn)
            assert count == 24
    asyncio.get_event_loop().run_until_complete(_run())
