"""Tests for reg_iso_30401 ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.reg_iso_30401 import NODES, ingest_reg_iso_30401


class TestRegIso30401Nodes:
...
def test_reg_iso_30401_module_importable():
    assert callable(ingest_reg_iso_30401)


def test_ingest_reg_iso_30401(db_pool):
    async def _run():
...
def test_ingest_reg_iso_30401_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_reg_iso_30401(conn)
            count = await ingest_reg_iso_30401(conn)
            assert count == 22
    asyncio.get_event_loop().run_until_complete(_run())
