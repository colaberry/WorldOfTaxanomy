"""Tests for domain_forex ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.domain_forex import NODES, ingest_domain_forex


class TestDomainForexNodes:
...
def test_domain_forex_module_importable():
    assert callable(ingest_domain_forex)


def test_ingest_domain_forex(db_pool):
    async def _run():
...
def test_ingest_domain_forex_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_domain_forex(conn)
            count = await ingest_domain_forex(conn)
            assert count == 15
    asyncio.get_event_loop().run_until_complete(_run())
