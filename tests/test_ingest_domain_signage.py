"""Tests for domain_signage ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.domain_signage import NODES, ingest_domain_signage


class TestDomainSignageNodes:
...
def test_domain_signage_module_importable():
    assert callable(ingest_domain_signage)


def test_ingest_domain_signage(db_pool):
    async def _run():
...
def test_ingest_domain_signage_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_domain_signage(conn)
            count = await ingest_domain_signage(conn)
            assert count == 16
    asyncio.get_event_loop().run_until_complete(_run())
