"""Tests for reg_wto_sps ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.reg_wto_sps import NODES, ingest_reg_wto_sps


class TestRegWtoSpsNodes:
...
def test_reg_wto_sps_module_importable():
    assert callable(ingest_reg_wto_sps)


def test_ingest_reg_wto_sps(db_pool):
    async def _run():
...
def test_ingest_reg_wto_sps_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_reg_wto_sps(conn)
            count = await ingest_reg_wto_sps(conn)
            assert count == 19
    asyncio.get_event_loop().run_until_complete(_run())
