"""Tests for reg_ifc_ps ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.reg_ifc_ps import NODES, ingest_reg_ifc_ps


class TestRegIfcPsNodes:
...
def test_reg_ifc_ps_module_importable():
    assert callable(ingest_reg_ifc_ps)


def test_ingest_reg_ifc_ps(db_pool):
    async def _run():
...
def test_ingest_reg_ifc_ps_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_reg_ifc_ps(conn)
            count = await ingest_reg_ifc_ps(conn)
            assert count == 21
    asyncio.get_event_loop().run_until_complete(_run())
