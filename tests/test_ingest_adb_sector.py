"""Tests for ADB Sector (adb_sector) ingester."""
...
class TestIngestAdbSector:
    ...
    def test_ingest_creates_system(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count = await ingest_adb_sector(conn)
                assert count >= 30, f"Expected >= 30 nodes, got {count}"

                sys = await _get_system(conn, "adb_sector")
                assert sys is not None
                assert sys["authority"] == "Asian Development Bank (ADB)"
    ...
    def test_ingest_creates_nodes(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count = await ingest_adb_sector(conn)
                db_count = await _count_nodes(conn, "adb_sector")
    ...
    def test_idempotent(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count1 = await ingest_adb_sector(conn)
                count2 = await ingest_adb_sector(conn)
                db_count = await _count_nodes(conn, "adb_sector")
