...
class TestIngestSfdr:
    ...
    def test_ingest_creates_system(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count = await ingest_sfdr(conn)
                assert count >= 25, f"Expected >= 25 nodes, got {count}"

                sys = await _get_system(conn, "sfdr")
                assert sys is not None
                assert sys["name"] == "SFDR"
    ...
    def test_ingest_creates_nodes(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count = await ingest_sfdr(conn)
                db_count = await _count_nodes(conn, "sfdr")
    ...
    def test_idempotent(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count1 = await ingest_sfdr(conn)
                count2 = await ingest_sfdr(conn)
                db_count = await _count_nodes(conn, "sfdr")
