"""Tests for reg_icao_annex ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.reg_icao_annex import NODES, ingest_reg_icao_annex


class TestRegIcaoAnnexNodes:
...
def test_reg_icao_annex_module_importable():
    assert callable(ingest_reg_icao_annex)


def test_ingest_reg_icao_annex(db_pool):
    async def _run():
...
def test_ingest_reg_icao_annex_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_reg_icao_annex(conn)
            count = await ingest_reg_icao_annex(conn)
            assert count == 26
    asyncio.get_event_loop().run_until_complete(_run())
