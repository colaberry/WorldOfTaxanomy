"""Tests for reg_equator ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.reg_equator import NODES, ingest_reg_equator


class TestRegEquatorNodes:
...
def test_reg_equator_module_importable():
    assert callable(ingest_reg_equator)


def test_ingest_reg_equator(db_pool):
    async def _run():
...
def test_ingest_reg_equator_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_reg_equator(conn)
            count = await ingest_reg_equator(conn)
            assert count == 18
    asyncio.get_event_loop().run_until_complete(_run())
