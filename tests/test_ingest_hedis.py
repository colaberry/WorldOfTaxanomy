"""Tests for HEDIS Healthcare Quality Measures ingester."""
import asyncio
from world_of_taxonomy.ingest.hedis import NODES, ingest_hedis


class TestStandaloneHedisNodes:
...
def test_hedis_module_importable():
    from world_of_taxonomy.ingest.hedis import ingest_hedis as fn
    assert callable(fn)


def test_ingest_hedis(db_pool):
    async def _run():
...
def test_ingest_hedis_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            c1 = await ingest_hedis(conn)
            c2 = await ingest_hedis(conn)
            assert c1 == c2
    asyncio.get_event_loop().run_until_complete(_run())
