"""Tests for reg_basel3 ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.reg_basel3 import NODES, ingest_reg_basel3


class TestRegBasel3Nodes:
...
def test_reg_basel3_module_importable():
    assert callable(ingest_reg_basel3)


def test_ingest_reg_basel3(db_pool):
    async def _run():
...
def test_ingest_reg_basel3_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_reg_basel3(conn)
            count = await ingest_reg_basel3(conn)
            assert count == 24
    asyncio.get_event_loop().run_until_complete(_run())
