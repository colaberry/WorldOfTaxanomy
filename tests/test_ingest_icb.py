...
class TestIngestIcb:
    ...
    def test_ingest_creates_system(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count = await ingest_icb(conn)
                assert count >= 20, f"Expected >= 20 nodes, got {count}"

                sys = await _get_system(conn, "icb")
                assert sys is not None
                assert sys["authority"] == "FTSE Russell"
    ...
    def test_ingest_creates_nodes(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count = await ingest_icb(conn)
                db_count = await _count_nodes(conn, "icb")
    ...
    def test_idempotent(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count1 = await ingest_icb(conn)
                count2 = await ingest_icb(conn)
                db_count = await _count_nodes(conn, "icb")
