"""Tests for domain_facade ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.domain_facade import NODES, ingest_domain_facade


class TestDomainFacadeNodes:
...
def test_domain_facade_module_importable():
    assert callable(ingest_domain_facade)


def test_ingest_domain_facade(db_pool):
    async def _run():
...
def test_ingest_domain_facade_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_domain_facade(conn)
            count = await ingest_domain_facade(conn)
            assert count == 16
    asyncio.get_event_loop().run_until_complete(_run())
