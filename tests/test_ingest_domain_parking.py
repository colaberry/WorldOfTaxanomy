"""Tests for domain_parking ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.domain_parking import NODES, ingest_domain_parking


class TestDomainParkingNodes:
...
def test_domain_parking_module_importable():
    assert callable(ingest_domain_parking)


def test_ingest_domain_parking(db_pool):
    async def _run():
...
def test_ingest_domain_parking_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_domain_parking(conn)
            count = await ingest_domain_parking(conn)
            assert count == 16
    asyncio.get_event_loop().run_until_complete(_run())
