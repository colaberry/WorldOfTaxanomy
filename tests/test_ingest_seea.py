...
class TestIngestSeea:
    ...
    def test_ingest_creates_system(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count = await ingest_seea(conn)
                assert count >= 40, f"Expected >= 40 nodes, got {count}"

                sys = await _get_system(conn, "seea")
                assert sys is not None
                assert sys["name"] == "SEEA"
    ...
    def test_ingest_creates_nodes(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count = await ingest_seea(conn)
                db_count = await _count_nodes(conn, "seea")
    ...
    def test_idempotent(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count1 = await ingest_seea(conn)
                count2 = await ingest_seea(conn)
                db_count = await _count_nodes(conn, "seea")
