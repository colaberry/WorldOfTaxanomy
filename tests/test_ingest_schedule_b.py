"""Tests for Schedule B (schedule_b) ingester."""
...
class TestIngestScheduleB:
    ...
    def test_ingest_creates_system(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count = await ingest_schedule_b(conn)
                assert count >= 100, f"Expected >= 100 nodes, got {count}"

                sys = await _get_system(conn, "schedule_b")
                assert sys is not None
                assert sys["authority"] == "US Census Bureau, Foreign Trade Division"
    ...
    def test_ingest_creates_nodes(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count = await ingest_schedule_b(conn)
                db_count = await _count_nodes(conn, "schedule_b")
    ...
    def test_idempotent(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count1 = await ingest_schedule_b(conn)
                count2 = await ingest_schedule_b(conn)
                db_count = await _count_nodes(conn, "schedule_b")
