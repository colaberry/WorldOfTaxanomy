"""Tests for reg_who_fctc ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.reg_who_fctc import NODES, ingest_reg_who_fctc


class TestRegWhoFctcNodes:
...
def test_reg_who_fctc_module_importable():
    assert callable(ingest_reg_who_fctc)


def test_ingest_reg_who_fctc(db_pool):
    async def _run():
...
def test_ingest_reg_who_fctc_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_reg_who_fctc(conn)
            count = await ingest_reg_who_fctc(conn)
            assert count == 18
    asyncio.get_event_loop().run_until_complete(_run())
