"""Tests for ICD-O-3 (icdo3) ingester."""
...
class TestIngestIcdo3:
    ...
    def test_ingest_creates_system(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count = await ingest_icdo3(conn)
                assert count >= 100, f"Expected >= 100 nodes, got {count}"

                sys = await _get_system(conn, "icdo3")
                assert sys is not None
                assert sys["authority"] == "World Health Organization (WHO)"
    ...
    def test_ingest_creates_nodes(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count = await ingest_icdo3(conn)
                db_count = await _count_nodes(conn, "icdo3")
    ...
    def test_idempotent(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count1 = await ingest_icdo3(conn)
                count2 = await ingest_icdo3(conn)
                db_count = await _count_nodes(conn, "icdo3")
