"""Tests for reg_uncitral ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.reg_uncitral import NODES, ingest_reg_uncitral


class TestRegUncitralNodes:
...
def test_reg_uncitral_module_importable():
    assert callable(ingest_reg_uncitral)


def test_ingest_reg_uncitral(db_pool):
    async def _run():
...
def test_ingest_reg_uncitral_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_reg_uncitral(conn)
            count = await ingest_reg_uncitral(conn)
            assert count == 20
    asyncio.get_event_loop().run_until_complete(_run())
