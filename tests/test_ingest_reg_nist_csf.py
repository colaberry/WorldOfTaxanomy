"""Tests for NIST CSF 2.0 regulatory taxonomy ingester.
...
from world_of_taxonomy.ingest.reg_nist_csf import (
    REG_NIST_CSF_NODES,
    ingest_reg_nist_csf,
)


class TestRegNistCsfNodes:
    def test_total_node_count(self):
        assert len(REG_NIST_CSF_NODES) == 28
    ...
    def test_no_duplicate_codes(self):
        codes = [n[0] for n in REG_NIST_CSF_NODES]
    ...
    def test_has_level_1_nodes(self):
        l1 = [n for n in REG_NIST_CSF_NODES if n[2] == 1]
        assert len(l1) >= 1


def test_reg_nist_csf_module_importable():
    assert callable(ingest_reg_nist_csf)
    assert isinstance(REG_NIST_CSF_NODES, list)


def test_ingest_reg_nist_csf(db_pool):
    """Integration test: ingest NIST CSF 2.0 taxonomy."""
    async def _run():
...
def test_ingest_reg_nist_csf_idempotent(db_pool):
    """Running ingest twice returns same count."""
    async def _run():
        async with db_pool.acquire() as conn:
            count1 = await ingest_reg_nist_csf(conn)
            count2 = await ingest_reg_nist_csf(conn)
