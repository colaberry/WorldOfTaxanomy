"""Tests for domain_zoning ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.domain_zoning import NODES, ingest_domain_zoning


class TestDomainZoningNodes:
...
def test_domain_zoning_module_importable():
    assert callable(ingest_domain_zoning)


def test_ingest_domain_zoning(db_pool):
    async def _run():
...
def test_ingest_domain_zoning_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_domain_zoning(conn)
            count = await ingest_domain_zoning(conn)
            assert count == 19
    asyncio.get_event_loop().run_until_complete(_run())
