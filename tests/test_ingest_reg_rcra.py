"""Tests for RCRA regulatory taxonomy ingester.
...
from world_of_taxonomy.ingest.reg_rcra import (
    REG_RCRA_NODES,
    ingest_reg_rcra,
)


class TestRegRcraNodes:
    def test_total_node_count(self):
        assert len(REG_RCRA_NODES) == 29
    ...
    def test_no_duplicate_codes(self):
        codes = [n[0] for n in REG_RCRA_NODES]
    ...
    def test_has_level_1_nodes(self):
        l1 = [n for n in REG_RCRA_NODES if n[2] == 1]
        assert len(l1) >= 1


def test_reg_rcra_module_importable():
    assert callable(ingest_reg_rcra)
    assert isinstance(REG_RCRA_NODES, list)


def test_ingest_reg_rcra(db_pool):
    """Integration test: ingest RCRA taxonomy."""
    async def _run():
...
def test_ingest_reg_rcra_idempotent(db_pool):
    """Running ingest twice returns same count."""
    async def _run():
        async with db_pool.acquire() as conn:
            count1 = await ingest_reg_rcra(conn)
            count2 = await ingest_reg_rcra(conn)
