"""Tests for reg_iso_13485 ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.reg_iso_13485 import NODES, ingest_reg_iso_13485


class TestRegIso13485Nodes:
...
def test_reg_iso_13485_module_importable():
    assert callable(ingest_reg_iso_13485)


def test_ingest_reg_iso_13485(db_pool):
    async def _run():
...
def test_ingest_reg_iso_13485_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_reg_iso_13485(conn)
            count = await ingest_reg_iso_13485(conn)
            assert count == 28
    asyncio.get_event_loop().run_until_complete(_run())
