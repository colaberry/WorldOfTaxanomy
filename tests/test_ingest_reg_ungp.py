"""Tests for reg_ungp ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.reg_ungp import NODES, ingest_reg_ungp


class TestRegUngpNodes:
...
def test_reg_ungp_module_importable():
    assert callable(ingest_reg_ungp)


def test_ingest_reg_ungp(db_pool):
    async def _run():
...
def test_ingest_reg_ungp_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_reg_ungp(conn)
            count = await ingest_reg_ungp(conn)
            assert count == 22
    asyncio.get_event_loop().run_until_complete(_run())
