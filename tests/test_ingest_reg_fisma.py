"""Tests for FISMA regulatory taxonomy ingester.
...
from world_of_taxonomy.ingest.reg_fisma import (
    REG_FISMA_NODES,
    ingest_reg_fisma,
)


class TestRegFismaNodes:
    def test_total_node_count(self):
        assert len(REG_FISMA_NODES) == 27
    ...
    def test_no_duplicate_codes(self):
        codes = [n[0] for n in REG_FISMA_NODES]
    ...
    def test_has_level_1_nodes(self):
        l1 = [n for n in REG_FISMA_NODES if n[2] == 1]
        assert len(l1) >= 1


def test_reg_fisma_module_importable():
    assert callable(ingest_reg_fisma)
    assert isinstance(REG_FISMA_NODES, list)


def test_ingest_reg_fisma(db_pool):
    """Integration test: ingest FISMA taxonomy."""
    async def _run():
...
def test_ingest_reg_fisma_idempotent(db_pool):
    """Running ingest twice returns same count."""
    async def _run():
        async with db_pool.acquire() as conn:
            count1 = await ingest_reg_fisma(conn)
            count2 = await ingest_reg_fisma(conn)
