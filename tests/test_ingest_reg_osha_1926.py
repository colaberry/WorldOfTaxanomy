"""Tests for OSHA 29 CFR 1926 regulatory taxonomy ingester.
...
from world_of_taxonomy.ingest.reg_osha_1926 import (
    REG_OSHA_1926_NODES,
    ingest_reg_osha_1926,
)


class TestRegOsha1926Nodes:
    def test_total_node_count(self):
        assert len(REG_OSHA_1926_NODES) == 49
    ...
    def test_no_duplicate_codes(self):
        codes = [n[0] for n in REG_OSHA_1926_NODES]
    ...
    def test_has_level_1_nodes(self):
        l1 = [n for n in REG_OSHA_1926_NODES if n[2] == 1]
        assert len(l1) >= 1


def test_reg_osha_1926_module_importable():
    assert callable(ingest_reg_osha_1926)
    assert isinstance(REG_OSHA_1926_NODES, list)


def test_ingest_reg_osha_1926(db_pool):
    """Integration test: ingest OSHA 29 CFR 1926 taxonomy."""
    async def _run():
...
def test_ingest_reg_osha_1926_idempotent(db_pool):
    """Running ingest twice returns same count."""
    async def _run():
        async with db_pool.acquire() as conn:
            count1 = await ingest_reg_osha_1926(conn)
            count2 = await ingest_reg_osha_1926(conn)
            assert count1 == count2 == 49
