"""Tests for CFI ISO 10962 (cfi_iso10962) ingester."""
...
class TestIngestCfiIso10962:
    ...
    def test_ingest_creates_system(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count = await ingest_cfi_iso10962(conn)
                assert count >= 50, f"Expected >= 50 nodes, got {count}"

                sys = await _get_system(conn, "cfi_iso10962")
                assert sys is not None
                assert sys["name"] == "CFI ISO 10962"
    ...
    def test_ingest_creates_nodes(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count = await ingest_cfi_iso10962(conn)
                db_count = await _count_nodes(conn, "cfi_iso10962")
    ...
    def test_idempotent(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count1 = await ingest_cfi_iso10962(conn)
                count2 = await ingest_cfi_iso10962(conn)
                db_count = await _count_nodes(conn, "cfi_iso10962")
