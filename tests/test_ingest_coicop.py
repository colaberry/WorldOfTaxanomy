"""Tests for COICOP 2018 (coicop) ingester."""
...
class TestIngestCoicop:
    ...
    def test_ingest_creates_system(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count = await ingest_coicop(conn)
                assert count >= 60, f"Expected >= 60 nodes, got {count}"

                sys = await _get_system(conn, "coicop")
                assert sys is not None
                assert sys["name"] == "COICOP 2018"
    ...
    def test_ingest_creates_nodes(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count = await ingest_coicop(conn)
                db_count = await _count_nodes(conn, "coicop")
    ...
    def test_idempotent(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count1 = await ingest_coicop(conn)
                count2 = await ingest_coicop(conn)
                db_count = await _count_nodes(conn, "coicop")
