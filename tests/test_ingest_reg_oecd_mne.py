"""Tests for reg_oecd_mne ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.reg_oecd_mne import NODES, ingest_reg_oecd_mne


class TestRegOecdMneNodes:
...
def test_reg_oecd_mne_module_importable():
    assert callable(ingest_reg_oecd_mne)


def test_ingest_reg_oecd_mne(db_pool):
    async def _run():
...
def test_ingest_reg_oecd_mne_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_reg_oecd_mne(conn)
            count = await ingest_reg_oecd_mne(conn)
            assert count == 22
    asyncio.get_event_loop().run_until_complete(_run())
