"""Tests for reg_iso_42001 ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.reg_iso_42001 import NODES, ingest_reg_iso_42001


class TestRegIso42001Nodes:
...
def test_reg_iso_42001_module_importable():
    assert callable(ingest_reg_iso_42001)


def test_ingest_reg_iso_42001(db_pool):
    async def _run():
...
def test_ingest_reg_iso_42001_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_reg_iso_42001(conn)
            count = await ingest_reg_iso_42001(conn)
            assert count == 32
    asyncio.get_event_loop().run_until_complete(_run())
