"""Tests for reg_iso_20000 ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.reg_iso_20000 import NODES, ingest_reg_iso_20000


class TestRegIso20000Nodes:
...
def test_reg_iso_20000_module_importable():
    assert callable(ingest_reg_iso_20000)


def test_ingest_reg_iso_20000(db_pool):
    async def _run():
...
def test_ingest_reg_iso_20000_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_reg_iso_20000(conn)
            count = await ingest_reg_iso_20000(conn)
            assert count == 23
    asyncio.get_event_loop().run_until_complete(_run())
