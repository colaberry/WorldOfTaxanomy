"""Tests for reg_iso_22301 ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.reg_iso_22301 import NODES, ingest_reg_iso_22301


class TestRegIso22301Nodes:
...
def test_reg_iso_22301_module_importable():
    assert callable(ingest_reg_iso_22301)


def test_ingest_reg_iso_22301(db_pool):
    async def _run():
...
def test_ingest_reg_iso_22301_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_reg_iso_22301(conn)
            count = await ingest_reg_iso_22301(conn)
            assert count == 26
    asyncio.get_event_loop().run_until_complete(_run())
