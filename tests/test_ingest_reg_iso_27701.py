"""Tests for reg_iso_27701 ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.reg_iso_27701 import NODES, ingest_reg_iso_27701


class TestRegIso27701Nodes:
...
def test_reg_iso_27701_module_importable():
    assert callable(ingest_reg_iso_27701)


def test_ingest_reg_iso_27701(db_pool):
    async def _run():
...
def test_ingest_reg_iso_27701_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_reg_iso_27701(conn)
            count = await ingest_reg_iso_27701(conn)
            assert count == 27
    asyncio.get_event_loop().run_until_complete(_run())
