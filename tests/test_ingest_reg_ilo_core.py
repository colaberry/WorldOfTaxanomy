"""Tests for reg_ilo_core ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.reg_ilo_core import NODES, ingest_reg_ilo_core


class TestRegIloCoreNodes:
...
def test_reg_ilo_core_module_importable():
    assert callable(ingest_reg_ilo_core)


def test_ingest_reg_ilo_core(db_pool):
    async def _run():
...
def test_ingest_reg_ilo_core_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_reg_ilo_core(conn)
            count = await ingest_reg_ilo_core(conn)
            assert count == 16
    asyncio.get_event_loop().run_until_complete(_run())
