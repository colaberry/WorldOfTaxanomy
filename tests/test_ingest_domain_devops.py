"""Tests for domain_devops ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.domain_devops import NODES, ingest_domain_devops


class TestDomainDevopsNodes:
...
def test_domain_devops_module_importable():
    assert callable(ingest_domain_devops)


def test_ingest_domain_devops(db_pool):
    async def _run():
...
def test_ingest_domain_devops_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_domain_devops(conn)
            count = await ingest_domain_devops(conn)
            assert count == 20
    asyncio.get_event_loop().run_until_complete(_run())
