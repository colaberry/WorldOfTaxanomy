"""Tests for reg_unclos ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.reg_unclos import NODES, ingest_reg_unclos


class TestRegUnclosNodes:
...
def test_reg_unclos_module_importable():
    assert callable(ingest_reg_unclos)


def test_ingest_reg_unclos(db_pool):
    async def _run():
...
def test_ingest_reg_unclos_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_reg_unclos(conn)
            count = await ingest_reg_unclos(conn)
            assert count == 25
    asyncio.get_event_loop().run_until_complete(_run())
