"""Tests for reg_berne ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.reg_berne import NODES, ingest_reg_berne


class TestRegBerneNodes:
...
def test_reg_berne_module_importable():
    assert callable(ingest_reg_berne)


def test_ingest_reg_berne(db_pool):
    async def _run():
...
def test_ingest_reg_berne_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_reg_berne(conn)
            count = await ingest_reg_berne(conn)
            assert count == 18
    asyncio.get_event_loop().run_until_complete(_run())
