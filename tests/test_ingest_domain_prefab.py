"""Tests for domain_prefab ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.domain_prefab import NODES, ingest_domain_prefab


class TestDomainPrefabNodes:
...
def test_domain_prefab_module_importable():
    assert callable(ingest_domain_prefab)


def test_ingest_domain_prefab(db_pool):
    async def _run():
...
def test_ingest_domain_prefab_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_domain_prefab(conn)
            count = await ingest_domain_prefab(conn)
            assert count == 14
    asyncio.get_event_loop().run_until_complete(_run())
