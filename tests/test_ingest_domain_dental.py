"""Tests for domain_dental ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.domain_dental import NODES, ingest_domain_dental


class TestDomainDentalNodes:
...
def test_domain_dental_module_importable():
    assert callable(ingest_domain_dental)


def test_ingest_domain_dental(db_pool):
    async def _run():
...
def test_ingest_domain_dental_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_domain_dental(conn)
            count = await ingest_domain_dental(conn)
            assert count == 18
    asyncio.get_event_loop().run_until_complete(_run())
