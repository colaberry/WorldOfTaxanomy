...
class TestIngestEccn:
    ...
    def test_ingest_creates_system(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count = await ingest_eccn(conn)
                assert count >= 50, f"Expected >= 50 nodes, got {count}"

                sys = await _get_system(conn, "eccn")
                assert sys is not None
                assert sys["authority"] == "Bureau of Industry and Security (BIS), US Dept of Commerce"
    ...
    def test_ingest_creates_nodes(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count = await ingest_eccn(conn)
                db_count = await _count_nodes(conn, "eccn")
    ...
    def test_idempotent(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count1 = await ingest_eccn(conn)
                count2 = await ingest_eccn(conn)
                db_count = await _count_nodes(conn, "eccn")
