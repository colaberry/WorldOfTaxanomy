"""Tests for NIST SP 800-53 Rev 5 regulatory taxonomy ingester.
...
from world_of_taxonomy.ingest.reg_nist_800_53 import (
    REG_NIST_800_53_NODES,
    ingest_reg_nist_800_53,
)


class TestRegNist80053Nodes:
    def test_total_node_count(self):
        assert len(REG_NIST_800_53_NODES) == 36
    ...
    def test_no_duplicate_codes(self):
        codes = [n[0] for n in REG_NIST_800_53_NODES]
    ...
    def test_has_level_1_nodes(self):
        l1 = [n for n in REG_NIST_800_53_NODES if n[2] == 1]
        assert len(l1) >= 1


def test_reg_nist_800_53_module_importable():
    assert callable(ingest_reg_nist_800_53)
    assert isinstance(REG_NIST_800_53_NODES, list)


def test_ingest_reg_nist_800_53(db_pool):
    """Integration test: ingest NIST SP 800-53 Rev 5 taxonomy."""
    async def _run():
...
def test_ingest_reg_nist_800_53_idempotent(db_pool):
    """Running ingest twice returns same count."""
    async def _run():
        async with db_pool.acquire() as conn:
            count1 = await ingest_reg_nist_800_53(conn)
            count2 = await ingest_reg_nist_800_53(conn)
