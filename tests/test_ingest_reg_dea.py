"""Tests for DEA Schedules regulatory taxonomy ingester.
...
from world_of_taxonomy.ingest.reg_dea import (
    REG_DEA_NODES,
    ingest_reg_dea,
)


class TestRegDeaNodes:
    def test_total_node_count(self):
        assert len(REG_DEA_NODES) == 25
    ...
    def test_no_duplicate_codes(self):
        codes = [n[0] for n in REG_DEA_NODES]
    ...
    def test_has_level_1_nodes(self):
        l1 = [n for n in REG_DEA_NODES if n[2] == 1]
        assert len(l1) >= 1


def test_reg_dea_module_importable():
    assert callable(ingest_reg_dea)
    assert isinstance(REG_DEA_NODES, list)


def test_ingest_reg_dea(db_pool):
    """Integration test: ingest DEA Schedules taxonomy."""
    async def _run():
...
def test_ingest_reg_dea_idempotent(db_pool):
    """Running ingest twice returns same count."""
    async def _run():
        async with db_pool.acquire() as conn:
            count1 = await ingest_reg_dea(conn)
            count2 = await ingest_reg_dea(conn)
