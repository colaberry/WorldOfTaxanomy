"""Tests for domain_hr_tech ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.domain_hr_tech import NODES, ingest_domain_hr_tech


class TestDomainHrTechNodes:
...
def test_domain_hr_tech_module_importable():
    assert callable(ingest_domain_hr_tech)


def test_ingest_domain_hr_tech(db_pool):
    async def _run():
...
def test_ingest_domain_hr_tech_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_domain_hr_tech(conn)
            count = await ingest_domain_hr_tech(conn)
            assert count == 18
    asyncio.get_event_loop().run_until_complete(_run())
