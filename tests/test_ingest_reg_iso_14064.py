"""Tests for reg_iso_14064 ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.reg_iso_14064 import NODES, ingest_reg_iso_14064


class TestRegIso14064Nodes:
...
def test_reg_iso_14064_module_importable():
    assert callable(ingest_reg_iso_14064)


def test_ingest_reg_iso_14064(db_pool):
    async def _run():
...
def test_ingest_reg_iso_14064_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_reg_iso_14064(conn)
            count = await ingest_reg_iso_14064(conn)
            assert count == 20
    asyncio.get_event_loop().run_until_complete(_run())
