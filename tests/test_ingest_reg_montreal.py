"""Tests for reg_montreal ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.reg_montreal import NODES, ingest_reg_montreal


class TestRegMontrealNodes:
...
def test_reg_montreal_module_importable():
    assert callable(ingest_reg_montreal)


def test_ingest_reg_montreal(db_pool):
    async def _run():
...
def test_ingest_reg_montreal_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_reg_montreal(conn)
            count = await ingest_reg_montreal(conn)
            assert count == 19
    asyncio.get_event_loop().run_until_complete(_run())
