"""Tests for Basel Exposure (basel_exposure) ingester."""
...
class TestIngestBaselExposure:
    ...
    def test_ingest_creates_system(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count = await ingest_basel_exposure(conn)
                assert count >= 30, f"Expected >= 30 nodes, got {count}"

                sys = await _get_system(conn, "basel_exposure")
                assert sys is not None
                assert sys["authority"] == "Bank for International Settlements (BIS/BCBS)"
    ...
    def test_ingest_creates_nodes(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count = await ingest_basel_exposure(conn)
                db_count = await _count_nodes(conn, "basel_exposure")
    ...
    def test_idempotent(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count1 = await ingest_basel_exposure(conn)
                count2 = await ingest_basel_exposure(conn)
                db_count = await _count_nodes(conn, "basel_exposure")
