"""Tests for reg_solas ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.reg_solas import NODES, ingest_reg_solas


class TestRegSolasNodes:
...
def test_reg_solas_module_importable():
    assert callable(ingest_reg_solas)


def test_ingest_reg_solas(db_pool):
    async def _run():
...
def test_ingest_reg_solas_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_reg_solas(conn)
            count = await ingest_reg_solas(conn)
            assert count == 21
    asyncio.get_event_loop().run_until_complete(_run())
