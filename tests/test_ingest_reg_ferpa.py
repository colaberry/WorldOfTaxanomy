"""Tests for FERPA regulatory taxonomy ingester.
...
from world_of_taxonomy.ingest.reg_ferpa import (
    REG_FERPA_NODES,
    ingest_reg_ferpa,
)


class TestRegFerpaNodes:
    def test_total_node_count(self):
        assert len(REG_FERPA_NODES) == 30
    ...
    def test_no_duplicate_codes(self):
        codes = [n[0] for n in REG_FERPA_NODES]
    ...
    def test_has_level_1_nodes(self):
        l1 = [n for n in REG_FERPA_NODES if n[2] == 1]
        assert len(l1) >= 1


def test_reg_ferpa_module_importable():
    assert callable(ingest_reg_ferpa)
    assert isinstance(REG_FERPA_NODES, list)


def test_ingest_reg_ferpa(db_pool):
    """Integration test: ingest FERPA taxonomy."""
    async def _run():
...
def test_ingest_reg_ferpa_idempotent(db_pool):
    """Running ingest twice returns same count."""
    async def _run():
        async with db_pool.acquire() as conn:
            count1 = await ingest_reg_ferpa(conn)
            count2 = await ingest_reg_ferpa(conn)
