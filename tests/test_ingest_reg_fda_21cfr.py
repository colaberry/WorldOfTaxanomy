"""Tests for FDA 21 CFR Parts regulatory taxonomy ingester.
...
from world_of_taxonomy.ingest.reg_fda_21cfr import (
    REG_FDA_21CFR_NODES,
    ingest_reg_fda_21cfr,
)


class TestRegFda21CfrNodes:
    def test_total_node_count(self):
        assert len(REG_FDA_21CFR_NODES) == 28
    ...
    def test_no_duplicate_codes(self):
        codes = [n[0] for n in REG_FDA_21CFR_NODES]
    ...
    def test_has_level_1_nodes(self):
        l1 = [n for n in REG_FDA_21CFR_NODES if n[2] == 1]
        assert len(l1) >= 1


def test_reg_fda_21cfr_module_importable():
    assert callable(ingest_reg_fda_21cfr)
    assert isinstance(REG_FDA_21CFR_NODES, list)


def test_ingest_reg_fda_21cfr(db_pool):
    """Integration test: ingest FDA 21 CFR Parts taxonomy."""
    async def _run():
...
def test_ingest_reg_fda_21cfr_idempotent(db_pool):
    """Running ingest twice returns same count."""
    async def _run():
        async with db_pool.acquire() as conn:
            count1 = await ingest_reg_fda_21cfr(conn)
            count2 = await ingest_reg_fda_21cfr(conn)
