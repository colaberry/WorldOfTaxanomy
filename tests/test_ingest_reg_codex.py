"""Tests for reg_codex ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.reg_codex import NODES, ingest_reg_codex


class TestRegCodexNodes:
...
def test_reg_codex_module_importable():
    assert callable(ingest_reg_codex)


def test_ingest_reg_codex(db_pool):
    async def _run():
...
def test_ingest_reg_codex_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_reg_codex(conn)
            count = await ingest_reg_codex(conn)
            assert count == 22
    asyncio.get_event_loop().run_until_complete(_run())
