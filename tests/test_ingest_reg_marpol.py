"""Tests for reg_marpol ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.reg_marpol import NODES, ingest_reg_marpol


class TestRegMarpolNodes:
...
def test_reg_marpol_module_importable():
    assert callable(ingest_reg_marpol)


def test_ingest_reg_marpol(db_pool):
    async def _run():
...
def test_ingest_reg_marpol_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_reg_marpol(conn)
            count = await ingest_reg_marpol(conn)
            assert count == 20
    asyncio.get_event_loop().run_until_complete(_run())
