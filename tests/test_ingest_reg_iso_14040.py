"""Tests for reg_iso_14040 ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.reg_iso_14040 import NODES, ingest_reg_iso_14040


class TestRegIso14040Nodes:
...
def test_reg_iso_14040_module_importable():
    assert callable(ingest_reg_iso_14040)


def test_ingest_reg_iso_14040(db_pool):
    async def _run():
...
def test_ingest_reg_iso_14040_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_reg_iso_14040(conn)
            count = await ingest_reg_iso_14040(conn)
            assert count == 25
    asyncio.get_event_loop().run_until_complete(_run())
