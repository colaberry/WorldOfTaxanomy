"""Tests for TSCA regulatory taxonomy ingester.
...
from world_of_taxonomy.ingest.reg_tsca import (
    REG_TSCA_NODES,
    ingest_reg_tsca,
)


class TestRegTscaNodes:
    def test_total_node_count(self):
        assert len(REG_TSCA_NODES) == 25
    ...
    def test_no_duplicate_codes(self):
        codes = [n[0] for n in REG_TSCA_NODES]
    ...
    def test_has_level_1_nodes(self):
        l1 = [n for n in REG_TSCA_NODES if n[2] == 1]
        assert len(l1) >= 1


def test_reg_tsca_module_importable():
    assert callable(ingest_reg_tsca)
    assert isinstance(REG_TSCA_NODES, list)


def test_ingest_reg_tsca(db_pool):
    """Integration test: ingest TSCA taxonomy."""
    async def _run():
...
def test_ingest_reg_tsca_idempotent(db_pool):
    """Running ingest twice returns same count."""
    async def _run():
        async with db_pool.acquire() as conn:
            count1 = await ingest_reg_tsca(conn)
            count2 = await ingest_reg_tsca(conn)
