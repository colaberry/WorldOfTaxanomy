"""Tests for DFARS regulatory taxonomy ingester.
...
from world_of_taxonomy.ingest.reg_dfars import (
    REG_DFARS_NODES,
    ingest_reg_dfars,
)


class TestRegDfarsNodes:
    def test_total_node_count(self):
        assert len(REG_DFARS_NODES) == 25
    ...
    def test_no_duplicate_codes(self):
        codes = [n[0] for n in REG_DFARS_NODES]
    ...
    def test_has_level_1_nodes(self):
        l1 = [n for n in REG_DFARS_NODES if n[2] == 1]
        assert len(l1) >= 1


def test_reg_dfars_module_importable():
    assert callable(ingest_reg_dfars)
    assert isinstance(REG_DFARS_NODES, list)


def test_ingest_reg_dfars(db_pool):
    """Integration test: ingest DFARS taxonomy."""
    async def _run():
...
def test_ingest_reg_dfars_idempotent(db_pool):
    """Running ingest twice returns same count."""
    async def _run():
        async with db_pool.acquire() as conn:
            count1 = await ingest_reg_dfars(conn)
            count2 = await ingest_reg_dfars(conn)
