"""Tests for NAIC Model Laws regulatory taxonomy ingester.
...
from world_of_taxonomy.ingest.reg_naic import (
    REG_NAIC_NODES,
    ingest_reg_naic,
)


class TestRegNaicNodes:
    def test_total_node_count(self):
        assert len(REG_NAIC_NODES) == 21
    ...
    def test_no_duplicate_codes(self):
        codes = [n[0] for n in REG_NAIC_NODES]
    ...
    def test_has_level_1_nodes(self):
        l1 = [n for n in REG_NAIC_NODES if n[2] == 1]
        assert len(l1) >= 1


def test_reg_naic_module_importable():
    assert callable(ingest_reg_naic)
    assert isinstance(REG_NAIC_NODES, list)


def test_ingest_reg_naic(db_pool):
    """Integration test: ingest NAIC Model Laws taxonomy."""
    async def _run():
...
def test_ingest_reg_naic_idempotent(db_pool):
    """Running ingest twice returns same count."""
    async def _run():
        async with db_pool.acquire() as conn:
            count1 = await ingest_reg_naic(conn)
            count2 = await ingest_reg_naic(conn)
