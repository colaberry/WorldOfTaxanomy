"""Tests for FORD (Frascati 2015) (ford_frascati) ingester."""
...
class TestIngestFordFrascati:
    ...
    def test_ingest_creates_system(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count = await ingest_ford_frascati(conn)
                assert count >= 48, f"Expected >= 48 nodes, got {count}"

                sys = await _get_system(conn, "ford_frascati")
                assert sys is not None
                assert sys["name"] == "FORD (Frascati 2015)"
    ...
    def test_ingest_creates_nodes(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count = await ingest_ford_frascati(conn)
                db_count = await _count_nodes(conn, "ford_frascati")
    ...
    def test_idempotent(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count1 = await ingest_ford_frascati(conn)
                count2 = await ingest_ford_frascati(conn)
                db_count = await _count_nodes(conn, "ford_frascati")
