"""Tests for ICD-10-GM (icd10_gm) ingester."""
...
class TestIngestIcd10Gm:
    ...
    def test_ingest_creates_system(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count = await ingest_icd10_gm(conn)
                assert count >= 30, f"Expected >= 30 nodes, got {count}"

                sys = await _get_system(conn, "icd10_gm")
                assert sys is not None
                assert sys["name"] == "ICD-10-GM"
    ...
    def test_ingest_creates_nodes(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count = await ingest_icd10_gm(conn)
                db_count = await _count_nodes(conn, "icd10_gm")
    ...
    def test_idempotent(self, db_pool):
        ...
        async def _test():
            async with db_pool.acquire() as conn:
                count1 = await ingest_icd10_gm(conn)
                count2 = await ingest_icd10_gm(conn)
                db_count = await _count_nodes(conn, "icd10_gm")
