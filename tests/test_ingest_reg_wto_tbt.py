"""Tests for reg_wto_tbt ingester."""
import asyncio
import pytest
from world_of_taxonomy.ingest.reg_wto_tbt import NODES, ingest_reg_wto_tbt


class TestRegWtoTbtNodes:
...
def test_reg_wto_tbt_module_importable():
    assert callable(ingest_reg_wto_tbt)


def test_ingest_reg_wto_tbt(db_pool):
    async def _run():
...
def test_ingest_reg_wto_tbt_idempotent(db_pool):
    async def _run():
        async with db_pool.acquire() as conn:
            await ingest_reg_wto_tbt(conn)
            count = await ingest_reg_wto_tbt(conn)
            assert count == 17
    asyncio.get_event_loop().run_until_complete(_run())
